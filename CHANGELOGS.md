...
- **Mid-stream connection drop retry (stream mode)** - Connection errors (`APIConnectionError`, `APITimeoutError`, `httpx.ReadError`, `httpx.TimeoutException`) raised during chunk iteration are now retried in streaming mode. Previously they were treated as non-retryable and emitted an incomplete SSE sequence (error event without `message_stop`), causing Claude Code to fail with "empty or malformed response (HTTP 200)".
- **Stream retry duplicate message_start** - Fixed a bug where a retried stream emitted a second `message_start` into the buffered SSE (one per attempt), leaving Claude Code with two message sequences and failing with "empty or malformed response (HTTP 200)". `stream_response` now keeps only the final attempt's events (from its last `message_start` onward), dropping partial earlier attempts.
- **stop_reason mismatch with streamed tool_use blocks** - When DeepSeek emits tool calls as DSML markup, OpenAI's `finish_reason` is `"stop"` even though the proxy streams `tool_use` content blocks. Claude Code requires `stop_reason="tool_use"` when tool_use blocks are present and rejected the delivered `end_turn` + tool_use message as "empty or malformed response (HTTP 200)". The stop_reason guards in both streaming and buffered paths now also upgrade `end_turn`/other → `tool_use` when tool_use blocks were actually streamed (tracked via a new `tool_blocks_started` counter in `SSEBuilder`).
...
- **Partial-response proxy notice** - Truncated/partial responses delivered to the client are appended with a visible notice (`[PROXY NOTICE] ... partial reply ... split the work into smaller chunks`) so the model knows the reply was cut short and can adapt instead of assuming the task finished.
