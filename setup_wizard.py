...
def _write_dotenv(path: Path, params: dict, is_linux: bool = False) -> None:
    ...
    lines = [
        ...
        f"REQUEST_QUEUE_DISCORD_PRIORITY={params.get('request_queue_discord_priority', 2)}",
        f"REQUEST_QUEUE_API_PRIORITY={params.get('request_queue_api_priority', 1)}",
        # NIM Settings
        ...
        # MCP Server settings
        f"MCP_BROWSER_HEADLESS={str(params.get('mcp_browser_headless', True)).lower()}",
        f"WEB_SEARCH_FETCH_TIMEOUT={params.get('mcp_fetch_timeout', 10.0)}",
        f"MCP_CACHE_TTL={params.get('mcp_cache_ttl', 600)}",
        f"WEB_SEARCH_DEBUG={str(params.get('web_search_debug', False)).lower()}",
        ...
        # Discord Browser Headless
        f"DISCORD_BROWSER_HEADLESS={str(params.get('discord_browser_headless', True)).lower()}",
        # Discord Web Search
        ...
        f"DISCORD_WEB_SEARCH_MAX_RESULT_SIZE={params.get('discord_web_search_max_result_size', 5000)}",
        f"DISCORD_WEB_SEARCH_INCLUDE_IN_HISTORY={str(params.get('discord_web_search_include_in_history', True)).lower()}",
    ]
    content = (
