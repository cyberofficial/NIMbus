...
def _write_dotenv(path: Path, params: dict, is_linux: bool = False) -> None:
    ...
    lines = [
        ...
        f'DISCORD_SYSTEM_PROMPT="{params.get("discord_system_prompt", "")}"',
...
def _run_section(
    ...
    if section == "nvidia_key":
        ...
        discord_conversation_category_id = ""
        discord_owner_id = ""
        ...
        if configure_discord:
            ...
            discord_guild_id = _prompt(
                ...
                default=str(existing.get("DISCORD_GUILD_ID", "")),
            )
            print()
            discord_control_channel_id = _prompt(
                "  Control Panel Channel ID (for bot commands)",
                default=str(existing.get("DISCORD_CONTROL_CHANNEL_ID", "")),
            )
            print()
            discord_conversation_category_id = _prompt(
                "  Conversation Category ID (bot creates channels here)",
                default=str(existing.get("DISCORD_CONVERSATION_CATEGORY_ID", "")),
            )
            print()
            discord_owner_id = _prompt(
                "  Your Discord User ID (right-click yourself -> Copy ID)",
                default=str(existing.get("DISCORD_OWNER_ID", "")),
            )
            print()
            ...
            discord_auto_compact = _prompt_yes_no(
                "  Auto-compact conversations when token limit near?", default=True
            )
        updates.update({
        ...
        if is_update:
            ...
            _write_dotenv(
                env_path,
                {
                    ...
                    # Discord Bot settings
                    "configure_discord": merged.get("DISCORD_ENABLED", "false") == "true",
                    "discord_token": merged.get("DISCORD_BOT_TOKEN", ""),
                    "discord_guild_id": merged.get("DISCORD_GUILD_ID", 0),
                    "discord_control_channel_id": merged.get("DISCORD_CONTROL_CHANNEL_ID", 0),
                    "discord_conversation_category_id": merged.get("DISCORD_CONVERSATION_CATEGORY_ID", 0),
                    "discord_owner_id": merged.get("DISCORD_OWNER_ID", 0),
                    "discord_owner_only": merged.get("DISCORD_OWNER_ONLY", "true") == "true",
                    "discord_system_prompt": merged.get("DISCORD_SYSTEM_PROMPT", ""),
                    "discord_auto_compact": merged.get("DISCORD_AUTO_COMPACT", "true") == "true",
                },
                is_linux=is_linux,
            )
