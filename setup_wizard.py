...
def _write_dotenv(path: Path, params: dict, is_linux: bool = False) -> None:
    ...
    lines = [
        ...
        # MCP Server settings
        f"WEB_SEARCH_MAX_RESULTS={params.get('mcp_max_results', 5)}",
        f"WEB_SEARCH_FETCH_TIMEOUT={params.get('mcp_fetch_timeout', 10.0)}",
        f"WEB_SEARCH_MAX_CHARS={params.get('mcp_max_chars', 10000)}",
    ]
    content = (
...
def _run_section(
    ...
    if section == "nvidia_key":
        ...
        if configure_mcp:
            print()
            mcp_max_results_str = _prompt(
                "  Max search results per query", default="5"
            )
            try:
                mcp_max_results = int(mcp_max_results_str)
            except ValueError:
                mcp_max_results = 5
            print()
            ...
            try:
                ...
                mcp_fetch_timeout = 10.0
            print()
            mcp_max_chars_str = _prompt(
                "  Max characters per fetched page", default="10000"
            )
            try:
                mcp_max_chars = int(mcp_max_chars_str)
            except ValueError:
                mcp_max_chars = 10000
        updates.update({
...
def _print_summary(params: dict, update_mode: bool = False, updated_sections: list | None = None) -> None:
    ...
    if params.get("configure_mcp"):
        ...
        print(f"  MCP Max Results:  {params.get('mcp_max_results', 5)}")
        print(f"  MCP Fetch Timeout: {params.get('mcp_fetch_timeout', 10.0)}s")
        print(f"  MCP Max Chars:  {params.get('mcp_max_chars', 10000)}")
        print()
        ...
        if is_update:
            ...
            _write_dotenv(
                env_path,
                {
                    ...
                    "mcp_max_results": int(merged.get("WEB_SEARCH_MAX_RESULTS", 5)),
                    "mcp_fetch_timeout": float(merged.get("WEB_SEARCH_FETCH_TIMEOUT", 10.0)),
                    "mcp_max_chars": int(merged.get("WEB_SEARCH_MAX_CHARS", 10000)),
                },
                is_linux=is_linux,
            )
            ...
            _print_summary(
                {
                    ...
                    "mcp_max_results": merged.get("mcp_max_results", 5),
                    "mcp_fetch_timeout": merged.get("mcp_fetch_timeout", 10.0),
                    "mcp_max_chars": merged.get("mcp_max_chars", 10000),
                    "is_linux": is_linux,
                    "exe_dir": exe_dir,
                },
                update_mode=True,
