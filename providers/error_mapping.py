...
def map_error(e: Exception) -> Exception:
    ...
    if isinstance(e, openai.RateLimitError):
        ...
        if hasattr(e, "response") and e.response is not None:
            ...
            # Parse retry-after header for accurate cooldown
            ...
            # Parse rate limit headers for dynamic updates
            GlobalRateLimiter.get_instance().parse_rate_limit_headers(headers)
            print(
                f"📤 Parsed rate limit headers on 429: retry_after={retry_seconds}s",
                flush=True,
            )

        GlobalRateLimiter.get_instance().set_blocked(retry_seconds)
